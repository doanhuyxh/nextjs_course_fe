  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadPromises = chunks.map((chunk, index) => {
        ...
        return fetch(`${process.env.MEDIA_UPLOAD_URL}/api/v1/upload/video_large_safe_host`, {
          method: 'POST',
          body: formData,
        }).then(res => {
      ...
      // gộp file tải lên
      const mergedFileResponse = await fetch(`${process.env.MEDIA_UPLOAD_URL}/api/v1/upload/merge_video_large_safe_host?fileName=${fileName}`)
